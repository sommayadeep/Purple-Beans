...
import { Star, ArrowLeft, Plus, Minus, Check, Sparkles, Coffee, Loader2 } from "lucide-react";
...
export default function ProductDetailsPage({ params }: { params: Promise<Params> }) {
  ...
  const finalPrice = product ? product.price * getWeightMultiplier(selectedWeight) : 0;

  const handleAddToCart = () => {
    if (!product) return;
    const adjustedProduct = {
      ...product,
      id: product.productId, // Match client store item id key
      price: finalPrice,
    };
